				...
				if ( ( scope.mode == 'translate' ) && isActive("T") ) {
					...
					if ( scope.space == 'local' ) {
						...
						if ( isActive("XYZ")) {
							...
							if ( isActive("X") ) scope.object.scale.x = oldScale.x * ( 1 + point.x / 10 );
							if ( isActive("Y") ) scope.object.scale.y = oldScale.y * ( 1 + point.y / 10 );
							if ( isActive("Z") ) scope.object.scale.z = oldScale.z * ( 1 + point.z / 10 );
